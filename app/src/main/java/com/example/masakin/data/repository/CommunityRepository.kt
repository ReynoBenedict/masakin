...
interface CommunityRepository {
    ...
    // imageUrl opsional, default null
...
// Dummy lokal, masih pakai demoPosts
...
/*
 * Repository utama: Firestore saja, tanpa Storage
 */
class FirestoreCommunityRepository(
    ...
    override suspend fun getRecommended(): List<Post> {
        val snap = postsRef
        ...
        val remote = snap.documents.mapNotNull { doc: DocumentSnapshot -> doc.toPost() }

        // kalau Firestore kosong, pakai dummy
        return if (remote.isNotEmpty()) remote else demoPosts.shuffled()
    }
    ...
    override suspend fun createPost(
        ...
        val data = mapOf(
            ...
            "content" to content,
            "imageUrl" to imageUrl,        // untuk user baru biasanya null
            "likes" to 0L,
            "comments" to 0L,
            "shares" to 0L,
    ...
    override suspend fun likePost(postId: String) {
        ...
        db.runTransaction { tx ->
            val snap = tx.get(doc)
            val currentLikes = snap.getLong("likes") ?: 0L
            tx.update(doc, "likes", currentLikes + 1L)
        }.await()
    }
}
...
private fun DocumentSnapshot.toPost(): Post? {
    ...
    val userHandle = getString("userHandle") ?: ""
    val userAvatarUrl = getString("userAvatarUrl") ?: ""
    val content = getString("content") ?: ""
    val imageUrl = getString("imageUrl")
    val likes = getLong("likes")?.toInt() ?: 0
    val comments = getLong("comments")?.toInt() ?: 0
    val shares = getLong("shares")?.toInt() ?: 0
    val createdAtMillis = getLong(FIELD_CREATED_AT) ?: 0L

    val timeAgo = if (createdAtMillis > 0L) createdAtMillis.toTimeAgo() else ""
